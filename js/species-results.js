...
(function(){
  ...
  function modelSpecies(sp,i){const pts=clean(sp),px=[];pts.forEach(p=>{const r=sample(p[0],p[1]);if(r)px.push(r);});if(px.length<C.min)return null;const seed=(C.seed+hash(sp.id||i))>>>0,b=bg(pts,seed);if(b.length<C.min)return null;const raw=px.concat(b),st=stats(raw),X=px.map(r=>z(st,r)).concat(b.map(r=>z(st,r))),y=new Array(px.length).fill(1).concat(new Array(b.length).fill(0)),m=fit(X,y),sc=X.map(r=>pred(m,r)),base=auc(sc,y),pp=px.map(r=>pred(m,z(st,r))),ranges=raw[0].map((_,j)=>({min:Math.min(...raw.map(r=>r[j])),max:Math.max(...raw.map(r=>r[j]))}));return{sp,st,m,threshold:thresh(sc,y),cv:cv(X,y,seed^0x9e3779b9),hsi:pp.reduce((a,b)=>a+b,0)/pp.length,imp:importance(X,y,m,base,seed^0x85ebca6b),med:px[0].map((_,j)=>median(px.map(r=>r[j]))),ranges};}
  function scenario(){
